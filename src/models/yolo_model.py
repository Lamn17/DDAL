...
def _normalized_class_probabilities(
    ...
    than a categorical softmax. DECS needs a categorical distribution to
...
def _raw_dfl_logits(preds, num_classes: int) -> Optional[torch.Tensor]:
    """Extract pre-integral DFL logits as ``(batch, 4 * reg_max, anchors)``.

    Ultralytics 8.4 returns ``(decoded, {boxes, scores, feats})`` in eval mode;
    older compatible releases return the raw per-scale tensors instead. Keep
    both layouts here so DDAL can fall back cleanly when the backend changes.
    """
    raw = preds[1] if isinstance(preds, (list, tuple)) and len(preds) > 1 else None
    if isinstance(raw, dict) and isinstance(raw.get("one2one"), dict):
        raw = raw["one2one"]
    if isinstance(raw, dict):
        boxes = raw.get("boxes")
        return boxes if isinstance(boxes, torch.Tensor) and boxes.ndim == 3 else None
    if isinstance(raw, (list, tuple)):
        box_maps = []
        for feature_map in raw:
            if not isinstance(feature_map, torch.Tensor) or feature_map.ndim != 4:
                continue
            box_channels = feature_map.shape[1] - num_classes
            if box_channels <= 4 or box_channels % 4:
                return None
            box_maps.append(
                feature_map[:, :box_channels].reshape(feature_map.shape[0], box_channels, -1)
            )
        return torch.cat(box_maps, dim=-1) if box_maps else None
    return None


def _normalized_dfl_entropy(
    raw_box_logits: Optional[torch.Tensor], kept_indices: torch.Tensor
) -> Optional[np.ndarray]:
    """Return mean four-side DFL entropy for post-NMS detections."""
    if raw_box_logits is None or raw_box_logits.ndim != 2:
        return None
    reg_max = raw_box_logits.shape[0] // 4
    if reg_max <= 1 or raw_box_logits.shape[0] != 4 * reg_max:
        return None
    if kept_indices.numel() == 0:
        return np.empty(0, dtype=np.float32)

    selected = raw_box_logits[:, kept_indices.long()].transpose(0, 1)
    distributions = selected.reshape(-1, 4, reg_max).softmax(dim=-1)
    entropy = -(
        distributions * distributions.clamp_min(torch.finfo(distributions.dtype).eps).log()
    ).sum(dim=-1) / np.log(reg_max)
    return entropy.mean(dim=1).detach().cpu().numpy()


def _end2end_class_scores_and_indices(
    preds,
    processed_predictions: torch.Tensor,
    num_classes: int,
    max_det: int,
    conf: float,
    classes=None,
):
    """Recover full class scores and anchor indices selected by an end-to-end head.

    YOLO26 performs top-k selection inside ``Detect.forward`` and exposes only
    ``(box, confidence, class)`` to the regular predictor. The accompanying raw
    output still contains the one-to-one score logits, so repeat the head's
    top-k indexing here and then apply the predictor's confidence/class filters.
    """
    raw = preds[1] if isinstance(preds, (list, tuple)) and len(preds) > 1 else None
    if isinstance(raw, dict) and isinstance(raw.get("one2one"), dict):
        raw = raw["one2one"]
    score_logits = raw.get("scores") if isinstance(raw, dict) else None
    if (
        not isinstance(score_logits, torch.Tensor)
        or score_logits.ndim != 3
        or score_logits.shape[1] != num_classes
        or processed_predictions.ndim != 3
        or processed_predictions.shape[0] != score_logits.shape[0]
    ):
        return None

    class_scores = score_logits.sigmoid()
    scores_by_anchor = class_scores.transpose(1, 2)
    batch_size, num_anchors, _ = scores_by_anchor.shape
    k = min(int(max_det), num_anchors)
    if processed_predictions.shape[1] != k:
        return None

    anchor_pool = scores_by_anchor.amax(dim=-1).topk(k, dim=1).indices
    pooled_scores = scores_by_anchor.gather(
        1, anchor_pool.unsqueeze(-1).expand(-1, -1, num_classes)
    )
    flat_indices = pooled_scores.flatten(1).topk(k, dim=1).indices
    selected_indices = anchor_pool.gather(1, flat_indices // num_classes)

    requested_classes = None
    if classes is not None:
        requested_classes = torch.as_tensor(
            classes, device=processed_predictions.device
        )

    recovered = []
    for batch_index in range(batch_size):
        prediction = processed_predictions[batch_index]
        indices = selected_indices[batch_index]
        keep = prediction[:, 4] > conf
        prediction = prediction[keep][:max_det]
        indices = indices[keep][:max_det]
        if requested_classes is not None:
            keep = (prediction[:, 5:6] == requested_classes).any(1)
            indices = indices[keep]
        recovered.append((class_scores[batch_index], indices))
    return recovered


class _ClassProbabilityDetectionPredictor(DetectionPredictor):
    """Detection predictor preserving class scores and DFL uncertainty.

    The stock ``Results.boxes`` API retains only ``(xyxy, confidence, class)``.
    This predictor tracks the candidates retained by standard NMS or by an
    end-to-end head's top-k selection and attaches their original class scores
    to each result as ``result.class_probs``.
    """
    ...
    def postprocess(self, preds, img, orig_imgs, **kwargs):
        ...
        if raw_predictions.shape[-1] == 6 or end2end:
            recovered = _end2end_class_scores_and_indices(
                preds,
                raw_predictions,
                num_classes,
                self.args.max_det,
            ...
            if recovered is None or len(recovered) != len(results):
                return results
            raw_box_logits = _raw_dfl_logits(preds, num_classes)
            for batch_index, (result, (scores, indices)) in enumerate(
                zip(results, recovered)
            ):
                result.class_probs = _normalized_class_probabilities(scores, indices)
                result.localization_uncertainty = _normalized_dfl_entropy(
                    raw_box_logits[batch_index] if raw_box_logits is not None else None,
                    indices,
                )
            return results

        if raw_predictions.shape[1] < 4 + num_classes:
            return super().postprocess(preds, img, orig_imgs, **kwargs)

        # Clone before NMS, which converts box coordinates in-place.
        class_scores = raw_predictions[:, 4:4 + num_classes, :].detach().clone()
        raw_box_logits = _raw_dfl_logits(preds, num_classes)
        if raw_box_logits is not None:
            raw_box_logits = raw_box_logits.detach().clone()
        ...
        for batch_index, (result, scores, indices) in enumerate(
            zip(results, class_scores, kept_indices)
        ):
            result.class_probs = _normalized_class_probabilities(scores, indices)
            result.localization_uncertainty = _normalized_dfl_entropy(
                raw_box_logits[batch_index] if raw_box_logits is not None else None,
...
class YOLOModel(BaseModel):
    ...
    def inference(self,
                  ...
                  return_localization_uncertainty: bool = False,
        ...
        use_ddal_predictor = return_class_probs or return_localization_uncertainty
        predictor = _ClassProbabilityDetectionPredictor if use_ddal_predictor else None
        using_probability_predictor = isinstance(
            getattr(self.model, "predictor", None), _ClassProbabilityDetectionPredictor
        )
        if use_ddal_predictor != using_probability_predictor:
        ...
        def convert_result(result) -> InferenceResult:
            ...
            if return_localization_uncertainty:
                value = getattr(result, "localization_uncertainty", None)
                if value is not None:
                    localization_uncertainty = np.asarray(value, dtype=float).reshape(-1)
                    if result.boxes is None or len(localization_uncertainty) != len(result.boxes):
                        raise RuntimeError(
                            "YOLO DFL uncertainty does not align with post-NMS detections"
                        )
