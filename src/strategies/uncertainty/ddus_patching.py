...
QUALITY_XI = 0.6
QUALITY_INITIAL_VALUES = None
QUALITY_INITIAL_CLS_VALUES = None
QUALITY_INITIAL_LOC_VALUES = None
QUALITY_TRACK_COMBINED = True


def configure_quality_tracking(
    momentum=0.99,
    initial_value=0.5,
    xi=0.6,
    initial_values=None,
    initial_cls_values=None,
    initial_loc_values=None,
    track_combined=True,
):
    global QUALITY_MOMENTUM, QUALITY_INITIAL_VALUE, QUALITY_XI
    global QUALITY_INITIAL_VALUES, QUALITY_INITIAL_CLS_VALUES, QUALITY_INITIAL_LOC_VALUES
    global QUALITY_TRACK_COMBINED
    ...
    QUALITY_XI = float(xi)
    QUALITY_TRACK_COMBINED = bool(track_combined)
    if initial_values is None:
        QUALITY_INITIAL_VALUES = None
    else:
        QUALITY_INITIAL_VALUES = np.asarray(initial_values, dtype=np.float32).reshape(-1)
    QUALITY_INITIAL_CLS_VALUES = (
        None
        if initial_cls_values is None
        else np.asarray(initial_cls_values, dtype=np.float32).reshape(-1)
    )
    QUALITY_INITIAL_LOC_VALUES = (
        None
        if initial_loc_values is None
        else np.asarray(initial_loc_values, dtype=np.float32).reshape(-1)
    )
...
def ddus_detection_loss(self, preds, batch):
    ...
    with torch.no_grad():
        for batch_idx in range(batch_size):
            ...
            combined = (
                pred_conf.pow(QUALITY_XI) * ious.pow(1.0 - QUALITY_XI)
                if QUALITY_TRACK_COMBINED
                else pred_conf
            )
            quality_rows.append(
                torch.stack(
...
def update_classwise_quality_ema(
    ...
    if QUALITY_TRACK_COMBINED:
        quality_updates.insert(0, ("class_quality", class_quality_sum))
    for attribute, value_sum in quality_updates:
...
def _on_train_start(trainer):
    ...
    class_quality = torch.full((num_classes,), initial, device=trainer.device)
    if QUALITY_INITIAL_VALUES is not None:
        previous = torch.as_tensor(QUALITY_INITIAL_VALUES, device=trainer.device)
        class_quality[: min(num_classes, len(previous))] = previous[:num_classes]
    trainer.class_quality = class_quality
    ...
    if QUALITY_INITIAL_CLS_VALUES is not None:
        previous = torch.as_tensor(QUALITY_INITIAL_CLS_VALUES, device=trainer.device)
        trainer.class_cls_quality[: min(num_classes, len(previous))] = previous[:num_classes]
    if QUALITY_INITIAL_LOC_VALUES is not None:
        previous = torch.as_tensor(QUALITY_INITIAL_LOC_VALUES, device=trainer.device)
        trainer.class_loc_quality[: min(num_classes, len(previous))] = previous[:num_classes]
...
def _on_train_epoch_end(trainer):
    ...
    quality_values = {
        ...
        "classwise_quality_cls": trainer.class_cls_quality.detach().clone(),
        "classwise_quality_loc": trainer.class_loc_quality.detach().clone(),
    }
    if QUALITY_TRACK_COMBINED:
        quality_values["classwise_quality"] = trainer.class_quality.detach().clone()
    for name, value in quality_values.items():
        setattr(model, name, value)
        np.save(Path(trainer.wdir) / f"{name}.npy", value.cpu().numpy())


def enable_class_quality_tracking(model):
    """Enable generic class-quality capture without replacing the trainer loop."""
...
def enable_ddus_tracking(model):
    """Backward-compatible DDUS entry point for class-quality tracking."""
    enable_class_quality_tracking(model)
