...
def parse_args():
    ...
    parser.add_argument('--strategy', type=str, help="Override strategy (this release supports ddal)")
    ...
    parser.add_argument('--override', action='append', help='Override nested config keys, e.g., --override strategy_args.ddal.sampling_conf=0.2')
...
def run_training(config: Dict, experiment_dir: str, round_num: int = 0) -> Dict:
    ...
    strategy = config.get('strategy', 'ddal')
    ...
    quality_cfg = strategy_args.get('ddal', strategy_args.get('ddus', {}))
    train_args.extend([
        '--quality_momentum', str(quality_cfg.get('quality_momentum', quality_cfg.get('momentum', 0.9))),
        '--quality_initial_value', str(quality_cfg.get('quality_initial', quality_cfg.get('initial_quality', 0.5))),
        '--quality_xi', str(quality_cfg.get('quality_xi', quality_cfg.get('xi', 0.5))),
    ])
    strategy_names = {item.strip() for item in strategy_str.replace(',', '-').split('-') if item.strip()}
    if 'ddal' in strategy_names and round_num > 0:
        previous_quality = sorted(
            (Path(experiment_dir) / f"round_{round_num - 1}" / "train").glob("*/weights/classwise_quality.npy")
        )
        if previous_quality:
            train_args.extend(['--quality_initial_path', str(previous_quality[-1])])
        previous_train_dir = (
            Path(experiment_dir) / f"round_{round_num - 1}" / "train"
        )
        previous_cls_quality = sorted(
            list(previous_train_dir.glob("*/weights/classwise_quality_cls.npy"))
            + list(previous_train_dir.glob("*/weights/classwise_cls_quality.npy"))
        )
        previous_loc_quality = sorted(
            list(previous_train_dir.glob("*/weights/classwise_quality_loc.npy"))
            + list(previous_train_dir.glob("*/weights/classwise_loc_quality.npy"))
        )
        if previous_cls_quality:
            train_args.extend(
                ['--quality_initial_cls_path', str(previous_cls_quality[-1])]
            )
        if previous_loc_quality:
            train_args.extend(
                ['--quality_initial_loc_path', str(previous_loc_quality[-1])]
            )
