...
def compare_with_seed_mean_std(
    ...
    if paper_style:
        ...
        style_by_name = {
            ...
            "CDAL": ("#9b59b6", "o", "white", 1.9),
            "DDAL": ("#ef4444", "s", "white", 1.9),
            "Ours": ("#ef4444", "s", "white", 1.9),
            "DDAL1": ("#c77728", "h", "white", 1.8),
            # ablation variants: distinct hue+marker per series, otherwise every
            # unnamed series falls back to one colour and the figure is unreadable
            "w/o DCQE": ("#ff7f0e", "d", "white", 1.6),
            "w/o BCQS": ("#9b59b6", "o", "white", 1.9),
            "Combined difficulty": ("#5b8c55", "^", "#5b8c55", 1.6),
            "cls only": ("#4aa3c0", "o", "#4aa3c0", 1.6),
            "loc only": ("#7f7f7f", "D", "white", 1.6),
            # signal_mode keeps a channel, so "w/o cls" is the loc-only run and
            # "w/o loc" is the cls-only run -- see scripts/plot_ablation.py
            "w/o cls": ("#4aa3c0", "o", "#4aa3c0", 1.6),
            "w/o loc": ("#7f7f7f", "D", "white", 1.6),
            "Full DDAL": ("#ef4444", "s", "white", 1.9),
            "Combined Difficulty": ("#5b8c55", "^", "#5b8c55", 1.6),
        }
    else:
        fig, ax = plt.subplots(figsize=(12, 8))
...
def plot_summary_mean_std(
    ...
    display_names = {
        "ddal": "DDAL", "badge": "BADGE", "random": "Random", "cdal": "CDAL",
    ...
    preferred_order = ["ddal", "random", "entropy", "badge", "cdal", "coreset"]
...
def plot_ablation_table(
    ...
    """Plot the first variants from a row-oriented ablation mean/std table."""
    ...
    if not rows:
        raise ValueError(f"Ablation table has no data rows: {path}")
    if limit <= 0:
        raise ValueError("limit must be positive")
    if "variant" not in fieldnames:
        raise ValueError(f"{path} must contain a variant column")

    rounds: List[int] = []
    while {f"r{len(rounds)}_mean", f"r{len(rounds)}_std"}.issubset(fieldnames):
        rounds.append(len(rounds))
    if not rounds:
        raise ValueError(f"{path} has no r<round>_mean/r<round>_std column pairs")

    selected_rows = rows[:limit]
    display_names = {
        "full": "Full DDAL",
        "without_dcqe": "w/o DCQE",
        "combined_difficulty": "Combined Difficulty",
        "without_bcqs": "w/o BCQS",
        "cls_only": "CLS-only",
        "loc_only": "LOC-only",
    }
    styles = {
    ...
    for index, row in enumerate(selected_rows):
        ...
        means = [float(row[f"r{round_number}_mean"]) for round_number in rounds]
        stds = [float(row[f"r{round_number}_std"]) for round_number in rounds]
        lower = [mean - std for mean, std in zip(means, stds)]
        upper = [mean + std for mean, std in zip(means, stds)]
        all_bounds.extend(lower + upper)
        label = display_names.get(variant, variant.replace("_", " "))
        color, marker, facecolor, edgewidth = styles.get(
            label, ("#4c72b0", "o", "#4c72b0", 1.6)
        )
        ax.fill_between(
            rounds, lower, upper, color=color, alpha=0.12, linewidth=0, zorder=1
        )
        ax.plot(
            rounds,
            means,
    ...
    ax.set_xlabel("Rounds", labelpad=12, fontweight="bold")
    ax.set_ylabel("mAP@50", labelpad=12, fontweight="bold")
    ax.set_xlim(rounds[0] - 0.35, rounds[-1] + 0.35)
    ax.set_xticks(rounds)
    if y_limits is not None:
        ax.set_ylim(*y_limits)
        ...
        padding = 0.025
        ax.set_ylim(min(all_bounds) - padding, max(all_bounds) + padding)
    ...
    ax.tick_params(width=1.6, length=8)
    ax.legend(
        loc="lower right", ncol=2, frameon=True, framealpha=0.94,
        edgecolor="#c8c8c8", fancybox=True, handlelength=2.25,
        columnspacing=1.1, handletextpad=0.4,
    )
    fig.tight_layout()
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        ...
        print("  plot-summary <summary.csv> --strategies ddal,badge,... [--paper-style] [--y-min 0.45 --y-max 0.83 --y-ticks 0.5,0.6,0.7,0.8] [--output output.png]")
        print("  plot-ablation <ablation.csv> [--limit 4] [--y-min 0.45 --y-max 0.83 --y-tick-step 0.1] [--output output.png]")
    ...
    elif command == "plot-ablation":
        parser = argparse.ArgumentParser(description="Plot a row-oriented ablation mean/std table")
        parser.add_argument("ablation_file", help="CSV with variant and rN_mean/rN_std columns")
        parser.add_argument("--limit", type=int, default=4, help="Number of variants from the top to plot")
        parser.add_argument("--y-min", type=float, help="Lower limit for the metric axis")
        parser.add_argument("--y-max", type=float, help="Upper limit for the metric axis")
        parser.add_argument("--y-tick-step", type=float, help="Spacing between metric-axis ticks")
        parser.add_argument("--output", default="", help="Output PNG path")
