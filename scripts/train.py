...
def parse_args():
    ...
    parser.add_argument('--quality_initial_path', type=str)
    parser.add_argument('--quality_initial_cls_path', type=str)
    parser.add_argument('--quality_initial_loc_path', type=str)
...
def main():
    ...
    try:
        ...
        quality_tracking_enabled = bool({"ddus", "ddal"} & strategies)
        if quality_tracking_enabled:
            from src.strategies.uncertainty.ddus_patching import (
                configure_quality_tracking,
                enable_class_quality_tracking,
            )

            quality_initial_values = None
            quality_initial_cls_values = None
            quality_initial_loc_values = None
            if args.quality_initial_path:
                quality_initial_values = np.load(args.quality_initial_path)
            if args.quality_initial_cls_path:
                quality_initial_cls_values = np.load(args.quality_initial_cls_path)
            if args.quality_initial_loc_path:
                quality_initial_loc_values = np.load(args.quality_initial_loc_path)
            configure_quality_tracking(
                momentum=args.quality_momentum,
                initial_value=args.quality_initial_value,
                xi=args.quality_xi,
                initial_values=quality_initial_values,
                initial_cls_values=quality_initial_cls_values,
                initial_loc_values=quality_initial_loc_values,
                track_combined="ddus" in strategies,
            )
        ...
        if quality_tracking_enabled:
            ...
            print(
                "Enabled class-quality tracking "
