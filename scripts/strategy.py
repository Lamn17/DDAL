...
from src.strategies import ReCALStrategy
...
def create_strategy(strategy_name: str, model: BaseModel, **kwargs):
    if strategy_name == 'recal':
        return ReCALStrategy(model, **kwargs)
    raise ValueError(f"Unknown strategy: {strategy_name}. This ReCAL release supports only 'recal'.")
