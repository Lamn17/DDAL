...
from src.strategies import DDALStrategy
...
def parse_args():
    ...
    parser.add_argument('--strategy', type=str, default='ddal',
                       help='Active learning strategy to use (this release supports ddal)')
...
def create_strategy(strategy_name: str, model: BaseModel, **kwargs):
    if strategy_name == 'ddal':
        return DDALStrategy(model, **kwargs)
    raise ValueError(f"Unknown strategy: {strategy_name}. This release supports only 'ddal'.")
...
def main():
    ...
    try:
        selected_indices = execute_chained_strategies(
            ...
            config=config,
            labeled_indices=data_manager.original_dataset.get_labeled_indices(),
            label_paths=data_manager.original_dataset.get_label_paths(),
        )
